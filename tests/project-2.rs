...
// `cavey get <KEY>` should print "Key not found" for a non-existent key and exit with zero.
#[test]
...
// `cavey rm <KEY>` should print "Key not found" for an empty database and exit with non-zero code.
#[test]
...
// `cavey put <KEY> <VALUE>` should print nothing and exit with zero.
#[test]
fn cli_put() {
    let temp_dir = TempDir::new().expect("unable to create temporary working directory");
    Command::cargo_bin("cavey")
...
fn cli_get_stored() -> Result<()> {
    let temp_dir = TempDir::new().expect("unable to create temporary working directory");
    let mut store = Cavey::open(temp_dir.path())?;
...
// `cavey rm <KEY>` should print nothing and exit with zero.
#[test]
...
fn overwrite_value() -> Result<()> {
    ...
    let mut store = Cavey::open(temp_dir.path())?;
    assert_eq!(store.get("key1".to_owned())?, Some("value2".to_owned()));
    store.put("key1".to_owned(), "value3".to_owned())?;
